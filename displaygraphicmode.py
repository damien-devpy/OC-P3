...
class DisplayGraphicMode:
	...
	def __init__(self,
		...
		self.screen = pygame.display.set_mode(
			(configuration.TOTAL_SIZE, configuration.TOTAL_SIZE+35)
		)
		...
		self._making_map(configuration.TOTAL_SIZE)
	...
	def counter(self):
		...
		for i, item in enumerate(self.items_object.items_carried):
			self.screen.blit(
				self.items_pict[item], (i*35, configuration.TOTAL_SIZE)
			)
		pygame.display.update()
	...
	def display_text(self, text):
		...
		counter_size = (len(configuration.ITEMS_TO_PICKED)
			* configuration.SIZE_SPRITES
			+ configuration.SIZE_SPRITES
		)
		...
		self.screen.blit(
			text_to_display, 
			(counter_size, configuration.TOTAL_SIZE+10)
		)
	...
	def _making_map(self, size_map):
		'''
		Building background and map objects regarding walls and hallways coordinates
		'''
